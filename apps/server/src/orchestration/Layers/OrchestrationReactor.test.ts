...
import { ThreadPrSettlementReactor } from "../Services/ThreadPrSettlementReactor.ts";
  ...
  it("starts every orchestration reactor", async () => {
    ...
    runtime = ManagedRuntime.make(
      Layer.effect(OrchestrationReactor, makeOrchestrationReactor).pipe(
        ...
        Layer.provideMerge(
          Layer.succeed(ThreadMicrovmLifecycleReactor, {
            start: () => {
              started.push("thread-microvm-lifecycle-reactor");
              return Effect.void;
            },
            drain: Effect.void,
          }),
        ...
        Layer.provideMerge(
          Layer.succeed(ThreadPrSettlementReactor, {
            start: () => {
              started.push("thread-pr-settlement-reactor");
              return Effect.void;
            },
            drain: Effect.void,
          }),
    ...
    expect(started).toEqual([
      ...
      "thread-microvm-lifecycle-reactor",
      "thread-pr-settlement-reactor",
