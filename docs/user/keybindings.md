...
`thread.copyId` copies the current thread's durable ID and defaults to `mod+shift+c`. You can use
that ID to reference the thread from another agent or T3 Code task.
